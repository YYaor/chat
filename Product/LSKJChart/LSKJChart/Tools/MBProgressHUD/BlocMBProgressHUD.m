...
@implementation BlocMBProgressHUD
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	BlocMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	BlocMBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
		return YES;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [BlocMBProgressHUD allHUDsForView:view];
	for (BlocMBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (MB_INSTANCETYPE)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (BlocMBProgressHUD *)subview;
		}
	}
	return nil;
}
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[BlocMBRoundProgressView class]];
	
    if (mode == MBProgressHUDRadio) {
        [indicator removeFromSuperview];
        
//        self.indicator = MB_AUTORELEASE([[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 50, 50)]);
//        self.indicator.contentMode = UIViewContentModeScaleAspectFit;
//        [(UIImageView *)self.indicator setImage:[UIImage bloc_voiceImageNamed:@"bloc_voice_HUD"]];
...
//        [self.indicator.layer addAnimation:animation forKey:nil];
    }
		...
		[indicator removeFromSuperview];
		self.indicator = MB_AUTORELEASE([[BlocMBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[BlocMBRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(BlocMBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
...
@implementation BlocMBRoundProgressView
...
@implementation BlocMBBarProgressView

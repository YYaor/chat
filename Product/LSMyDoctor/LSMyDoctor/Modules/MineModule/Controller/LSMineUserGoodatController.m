...
@interface LSMineUserGoodatController ()
...
@implementation LSMineUserGoodatController
...
- (void)initForView
{
    self.navigationItem.title = @"编辑擅长";
    ...
    self.infoTextView.text = self.model.myBaseInfo[5][@"value"];
}
...
-(void)sureButtonClick{
    ...
    {
        [XHToast showCenterWithText:@"请输入您的擅长"];
        return;
    }
    ...
    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:arr[5]];
    [dic setValue:self.infoTextView.text forKey:@"value"];
    [arr replaceObjectAtIndex:5 withObject:dic];
...
-(YYPlaceholderTextView *)infoTextView
{
    if (!_infoTextView)
    {
        ...
        _infoTextView.placeholder = @"请输入您的擅长";

...
- (void)setXLineTitleLabel{
    for (NSInteger i = 0; i < self.xLineTitleArray.count; i++) {
        CGFloat lineItemGapLength = 22;//隔空
        if (self.xLineTitleArray.count == 4) {
            lineItemGapLength = 31;
        }else if (self.xLineTitleArray.count == 3){
            lineItemGapLength = 38;
        }
        
        CGFloat xPos = self.axisLine.xLineStartXPos + lineItemGapLength + (XLineItemWidth + lineItemGapLength) * i + 7;
        CGFloat yPos = self.axisLine.xLineStartYPos + 5;
        CGFloat width = XLineItemWidth;
        CGFloat height = _xLineLabelHeight;
        
        //label的中心点
        CGPoint label_center = CGPointMake(xPos + width * 0.5 - 20, yPos + height * 0.5);
        ...
        ZFLabel * label = [[ZFLabel alloc] initWithFrame:CGRectMake(0, 0, rect.size.width, rect.size.height+40)];

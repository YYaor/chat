...
- (void)drawRect:(CGRect)rect
{
    if (_dataArray) {
        ...
        for (PieCharModel *model in _dataArray) {
            ...
            NSDictionary *dict = @{NSFontAttributeName:[UIFont systemFontOfSize:self.textFont - 2]};
            ...
            if (radius_start + radius_end / 2 < 90 || radius_start + radius_end / 2 > 270) { // 右边的线
                ...
                label_x  = line_x + size.width / 2 + 10;
            ...
            // 显示框
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, size.width + 20, 20)];
            label.text = labelTitle;
            label.textAlignment = NSTextAlignmentCenter;
            label.font = [UIFont systemFontOfSize:self.textFont];
            label.textColor = _textColor;
            [self addSubview:label];
            label.center = CGPointMake(label_x, point2.y);
            
//            if ([self.reportTypeStr isEqualToString:@"T04"]) {
//                //圆环
//                drawArc(ctx, self.center, radians(radius_start), radians(radius_start + radius_end), [UIColor yellowColor], self.radius*0.8);
//                
//            }
            ...
            imgView.image = [UIImage imageNamed:@"yourposition"];
            imgView.contentMode = UIViewContentModeScaleAspectFit;
            imgView.hidden = model.yourPositionHidden;
            [self addSubview:imgView];
            [imgView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerX.equalTo(label.mas_centerX);
                make.bottom.equalTo(label.mas_top).offset(2);
